...
addon_id        = 'plugin.video.motorreplays'
...
baseurl         = 'http://aleightyrepo.comli.com/Addons/MotorReplays/base.xml'
...
messagetext     ='http://aleightyrepo.comli.com/Addons/MotorReplays/info.xml'
...
def GetMenu():
        ...
        for item in match:
            ...
            view(link)
        addDir('[B][COLOR yellow]News and Updates[/COLOR][/B]',url,10,whatsnewicon,fanarts)
        addDir('[B][COLOR yellow]Search[/COLOR][/B]',url,5,searchicon,fanarts)
...
def popup():
        ...
        if len(message)>1:
                ...
                if compfile == message:pass
                        ...
                        showText('[B][COLOR yellow]Motor Replays[/COLOR] [COLOR white]News and Updates[/COLOR][/B]', message)
...
def SEARCH():
	keyb = xbmc.Keyboard('', '[COLOR white]Search[/COLOR] [COLOR yellow]Motor Replays[/COLOR]')
	keyb.doModal()
	if (keyb.isConfirmed()):
		searchterm=keyb.getText()
		searchterm=searchterm.upper()
	else:quit()
	link=open_url('http://aleightyrepo.comli.com/Addons/MotorReplays/search.xml')
