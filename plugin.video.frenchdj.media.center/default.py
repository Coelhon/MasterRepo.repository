...
addon_id = 'plugin.video.frenchdj.media.center'
ADDON = xbmcaddon.Addon(id=addon_id)
AddonID='plugin.video.frenchdj.media.center'
AddonTitle="FMC"
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.live.frenchdj.media.center Lists-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(frenchdj,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(frenchdj,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")

				
def SKindex():
    addon_log("SKindex")
    addDir('[B][COLOR deepskyblue][ Favoritos ][/COLOR][/B]','[B][COLOR deepskyblue][ Favoritos ][/COLOR][/B]',4,'http://i.imgur.com/r6OTzHx.png' ,  FANART,'http://imgur.com/WdWFWqh.gif','http://imgur.com/WdWFWqh.gif','http://imgur.com/WdWFWqh.gif','http://imgur.com/WdWFWqh.gif')
    getData(Base,FANART)
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(frenchdj,New source added.,5000,"+icon+")")
        if not url is None:
...
def GetSublinks(name,url,iconimage,fanart):
    ...
    if c==1:
        try:
         ...
         rNo=dialog.select('frenchdj Select A Source', List)
         if rNo>=0:
             rName=name
             rURL=str(ListU[rNo])
...
#hakamac code
    ...
    progress.create('frenchdj Searching Please wait',' ')
...
def urlsolver(url):
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(frenchdj,Please enable Update Commonresolvers to Play in Settings. - ,10000)")

    resolved=genesisresolvers.get(url).result
    if url == resolved or resolved is None:
        #import
        xbmc.executebuiltin("XBMC.Notification([B][COLOR lime]FMC[/COLOR][/B],[COLOR white][B]A usar Urlresolver module[/COLOR][/B] - ,5000)")
        import urlresolver
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('frenchdj','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('frenchdj', 'Do you want to add this file as a source?')
        if ret:
...
def addDir(name,url,mode,iconimage,fanart,description,genre,date,credits,showcontext=False):
        ...
        if showcontext:
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('[COLOR white]Remover de [B][COLOR lime]FMC[/COLOR][/B][COLOR white] Favoritos[/COLOR]','XBMC.RunPlugin(%s?mode=6&name=%s)'
                                    %(sys.argv[0], urllib.quote_plus(name))))
									
            if not name in FAV:
                contextMenu.append(('[COLOR white]Adicionar a [B][COLOR lime]FMC[/COLOR][/B][COLOR white] Favoritos[/COLOR]','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
def search(site_name,search_term=None):
    ...
    if 'youtube' in site_name:
        ...
        if res == 'True':
            ...
            xbmc.executebuiltin("XBMC.Notification(frenchdj,No IMDB match found ,7000,"+icon+")")
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie=""):
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('[COLOR white]Remover de [B][COLOR lime]FMC[/COLOR][/B][COLOR white] Favoritos[/COLOR]','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('[COLOR white]Adicionar a [B][COLOR lime]FMC[/COLOR][/B][COLOR white] Favoritos[/COLOR]','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
...
params=get_params()
Base = 'aHR0cDovL2ZtYy5wZS5odS9mbWMucGhwP3VzZXI9'.decode('base64')+USER+'JnBhc3M9'.decode('base64')+PASS
url=None
name=None
mode=None
playlist=None
...
if mode==None:
    ...
    if url:
        ...
        xbmc.executebuiltin("XBMC.Notification(frenchdj ,Failed to extract regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification(frenchdj,Please [COLOR yellow]install the Youtube Addon[/COLOR] module ,10000,"")")
